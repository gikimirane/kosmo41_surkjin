...
%>
  ...
  <div class="collapse navbar-collapse" id="navbarTogglerDemo01">
	...
	<input id="setblk" type="text" class="form-control"  name="setblk"  placeholder="대상자를 입력/선택 하세요">
	 ...
	 <div align="center">
		<input type="button" class="btn btn-primary" value="확인" onclick="blocking();"/> &nbsp; &nbsp;
	...
	<input id="call1" type="text" class="form-control"  name="call1"  placeholder="대기실내에서 선택 또는 입력하세요">
	 ...
	 <div align="center">
		<input type="button" class="btn btn-primary" value="확인" onclick="setTalkToOne(2,talkOn)"/> &nbsp; &nbsp;
	...
	<input id="call2" type="text" class="form-control"  name="call2"  placeholder="대화방내에서 선택 또는 입력하세요">
	 ...
	 <div align="center">
		<input type="button" class="btn btn-primary" value="확인" onclick="setTalkToOne(4,talkOn)"/> &nbsp; &nbsp;
	...
	<input id="outMan" type="text" class="form-control"  name="outMan"  placeholder="대화방내에서 선택 또는 입력하세요">
	 ...
	 <div align="center">
		<input type="button" class="btn btn-primary" value="강퇴" onclick="outRoom();"/> &nbsp; &nbsp;
	...
	<input id="setblk1" type="text" class="form-control"  name="setblk1"  placeholder="대상자를 입력/선택 하세요">
	 ...
	 <div align="center">
		<input type="button" class="btn btn-primary" value="확인" onclick="blocking1();"/> &nbsp; &nbsp;
	...
	<div id="delrm" class="input-group mb-3">
	 폭파하시겠습니까?</div>
	 <div align="center">
		<input type="button" class="btn btn-primary" value="확인" onclick="deleteRoom();"/> &nbsp; &nbsp;
	...
	<script type="text/javascript">
		...
		var mbr_id, talk_id, game_cd, click_no, roomOrder;
		var id = "<%= id %>";
		...
		function openSocket(){
			...
			webSocket.onclose = function(event) {
				...
				selected1.innerHTML = "";
				selected2.innerHTML = "";
				headTitle.innerHTML =  id + " 님 반갑습니다.";
			};
		}
		...
		function setTalkToOne(clickNo, onoff){
			if(chkWebSocket()==false) return;
			talk_id = (clickNo==2 ? call1.value : call2.value);
			roomOrder = 0;
			if(clickNo != click_no){
				if(clickNo==2){
				...
				return;
			}else if(onoff==1 && (talk_id==null || talk_id=="" || talk_id==id)){
				document.getElementById(clickNo==2 ? 'selected1':'selected2').innerHTML = "귓속말할 사람을 선택하세요.";
				return;
			}
						
			talkTo = onoff;
			document.getElementById(clickNo==2 ? 'selected1':'selected2').innerHTML = 
				talk_id + (onoff==1 ? "에 대한 귓속말을 설정했습니다.":"에 대한 귓속말을 해제했습니다.");	
			document.getElementById(clickNo==2 ? 'callto1':'callto2').style.display ="none";
		}
		...
		function writeResponse(text){
			...
			if(str[0] == "0"){ //open
				...
				webSocket.send(id +" 7 "+ " 님이 입장하셨습니다.");
				waitList();
			}
			else if(str[0] == "1") //대기실
			...
			else if(str[0] == "2" || str[0] == "7" || str[0] == "8"){ //채팅 //7:입장 8:방생성
		...
		function fclick(bcd, hid){
			...
			if(bcd==2){ 	
				if(roomOrder== 1)	callMan.value = hid;
				else 				call1.value = hid;
									setblk.value = hid;
			}
			if(bcd==3){
				room_no = hid;
				...
				webSocket.send(id +" 1 " + "/cd " + hid);
				headTitle.innerHTML = id + " (" + hid + "번방)";
				myRoomList();
				roomList(room_cd);
			}
			else if(bcd==4){
				if(roomOrder == 0) 		call2.value = hid;
				else if(roomOrder ==1)  callMan.value = hid;
				else if(roomOrder ==2)  outMan.value = hid;
				else if(roomOrder ==3)  changeMan.value = hid;
			}else if(bcd==5) 			ban.value = hid;
			 else if(bcd==6) 			setblk.value = hid;
		}
